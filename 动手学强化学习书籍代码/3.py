...
Game='CartPole-v0'
N_workers=multiprocessing.cpu_count()    #独立玩家个体数为cpu线程数
        ...
        while not COORD.should_stop() and GLOBALE_EP<MAX_GLOBAL_EP:   #停止本worker运行的条件
            ...
            while True:      #本循环一次是一步
                ...
                if total_step % UPDATE_GLOBALE_ITER==0 or done:  #每iter步完了或者或者到达终点了，进行同步sync操作
                    ...
                    buffer_s,buffer_a,buffer_v_target=np.vstack(buffer_s),np.vstack(buffer_a),np.vstack(buffer_v_target) # 竖向堆叠
...
if __name__=='__main__':
    ...
    with tf.device('/cpu:0'):
        ...
        COORD=tf.train.Coordinator()    #多线程管理 可以用来同时停止多个工作线程
        SESS.run(tf.global_variables_initializer())   #初始化所有参数 变为tensor

        worker_threads=[]
        for worker in workers:    #并行过程
            job= lambda:worker.work()   #worker的工作目标,此处调用Worker类中的work  定义无参数函数job()
            t=threading.Thread(target=job)  #创建线程 每一个线程完成一个worker的工作目标 
            t.start()                 # 启动每一个worker
            worker_threads.append(t)   #每一个worker的工作都加入thread中
        COORD.join(worker_threads)     #合并几个worker,当每一个worker都运行完再继续后面步骤  把线程加入主线程，等待线程结束
