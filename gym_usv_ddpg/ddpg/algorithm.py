...
class DDPG(parl.Algorithm):
    def __init__(self,
        ...
        self.model = model
        # 复制value()和policy()网络
        # deepcopy()复制网络结构，parameters()复制网络参数名称，sync_weights_to（）复制参数值
    ...
    def _critic_learn(self, obs, action, reward, next_obs, terminal):
        with paddle.no_grad():
            # 计算 target Q: 用value()计算Q'，用policy()计算next_action
            # 因为要固定的只是Q'和a',因此用target_model网络更新Q'和a',即value()和policy
            # 其他的值就用model网络更新计算，最后用sync_weights_t更新target_model网络参数
    ...
    def _actor_learn(self, obs):
        # 计算 Actor loss=-Q 最小化loss就是最大化Q,对应最好的a
