...
MEMORY_WARMUP_SIZE = MEMORY_SIZE //  20 # 预存一部分经验之后再开始训练,1/20 size
BATCH_SIZE = 128
...
def run_train_episode(agent, env, rpm):
    ...
    while True:
        steps += 1
        # batch_obs=[[,,,]]即状态是4个， rpm的batch_obs=[[],..,[]] 只是命名重了
        # 根据模型输入要求维度1*...
        """TensorFlow中，想要维度增加一维，可以使用tf.expand_dims(input, dim, name=None)函数。
           当然，我们常用tf.reshape(input, shape=[])也可以达到相同效果，但是有些时候在构建图的过程中，
           placeholder没有被feed具体的值，
...
def main():
    ...
    while len(rpm) < MEMORY_WARMUP_SIZE:
        # if len(rpm) > MEMORY_WARMUP_SIZE and (steps % 5) == 0 因此不会使参数更新
