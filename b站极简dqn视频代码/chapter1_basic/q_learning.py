...
import numpy as np
import gridworld
# 基于sarsa的保守，将Q(s',a')改为maxQ(s',A)，用其四周最大的格子更新Q（期望价值），这样训练时不会走到悬崖，最后会贴着悬崖走，
class QLearningAgent(object):
    ...
    def predict(self, state):
        #return np.argmax(self.Q[state, :]) 仅会采取第一个最大的下标，这里取某一行
        Q_list = self.Q[state, :]
